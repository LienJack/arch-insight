...
- 上下文打包工具（`context-pack`）是上下文准备工具，不是分析工具；正确顺序是先 intake 再决定是否打包。`context-pack` 产出（如 `outputs/context-pack.md`）是辅助性中间文件，不属于本契约定义的主报告与学习附件体系
- `RUNNER.md` 在本契约的默认路径与生态扩展路径基础上，细化了面向大仓库/上下文受限场景（路径 B）和知识沉淀/Onboarding 场景（路径 D）的操作变体；这些变体不引入新的能力阶段，仅调整执行策略。
